...
### POST METHOD
def test_post_user():
    r = requests.put('http://' + HOST + ':' + PORT + '/hello/' + USER, data={'dateOfBirth': '1998-01-01'})
    assert r.status_code == 204
    assert r.text == ''
    requests.delete('http://' + HOST + ':' + PORT + '/hello/' + USER)

def test_post_user_with_invalid_username():
    r = requests.put('http://' + HOST + ':' + PORT + '/hello/n3w-us4r', data={'dateOfBirth': '1998-01-01'})
    assert r.status_code == 400
    assert r.json()["message"] == "The username can only contain letters, received: n3w-us4r"

def test_post_user_wrong_date():
    r = requests.put('http://' + HOST + ':' + PORT + '/hello/' + USER, data={'dateOfBirth': '2050-01-01'})
    assert r.status_code == 400
...
def test_post_user_incorrect_date_format():
    r = requests.put('http://' + HOST + ':' + PORT + '/hello/' + USER, data={'dateOfBirth': '1998-00-00'})
    assert r.status_code == 400
...
def test_get_user():
    ...
    if day == TODAY.day:
        day = 12
    r = requests.put('http://' + HOST + ':' + PORT + '/hello/' + USER, data={'dateOfBirth': date(TODAY.year, TODAY.month, day).isoformat()})
    assert r.status_code == 204
    r = requests.get('http://' + HOST + ':' + PORT + '/hello/' + USER)
    assert r.status_code == 200
    assert "Hello, " + USER + "! Your birthday is in" in r.json()["message"]
    requests.delete('http://' + HOST + ':' + PORT + '/hello/' + USER)

def test_get_user_with_birthday():
    r = requests.put('http://' + HOST + ':' + PORT + '/hello/' + USER, data={'dateOfBirth': date(TODAY.year-20, TODAY.month, TODAY.day).isoformat()})
    r = requests.get('http://' + HOST + ':' + PORT + '/hello/' + USER)
    assert r.status_code == 200
...
def test_get_non_existent_user():
    r = requests.get('http://' + HOST + ':' + PORT + '/hello/nonexistentuser')
